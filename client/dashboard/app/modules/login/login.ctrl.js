define([
    ...
    function loginCtrl($state, $stateParams, $timeout, $uibModal, Restangular, $window, $sce, $scope) {
        ...
        vm.authUser = function() {
            if(vm.userEmail && vm.userPassword) {
                vm.userErr = false;
                loginHelper.authenticateUser(vm.userEmail, MD5(vm.userPassword)).then(function(response) {
                    if(!response.status) {
    ...
    loginCtrl.$inject = ['$state', '$stateParams', '$timeout', '$uibModal', 'Restangular', '$window', '$sce', '$cookies'];
